...
async def get_user_balance(user_id: int, session: AsyncSession) -> List[UserBalance]:
    ...
    for currency in all_currencies:
        statement = select(UserBalance).where(
            (UserBalance.user_id == user_id) & (UserBalance.currency_code == currency.code)
        )
        result = await session.exec(statement)
        balance = result.first()
        if not balance:
            balance = UserBalance(user_id=user_id, currency_code=currency.code, balance=0.0)
            session.add(balance)
...
async def get_user_balance_by_currency_code(user_id: int, currency_code: str, session: AsyncSession) -> UserBalance:
    statement = select(UserBalance).where(
        (UserBalance.user_id == user_id) & (UserBalance.currency_code == currency_code)
    )
    result = await session.exec(statement)
    balance = result.first()
    if not balance:
        balance = UserBalance(user_id=user_id, currency_code=currency_code, balance=0.0)
        session.add(balance)
        await session.commit()
        await session.refresh(balance)
    return balance
...
async def deposit_funds(user_id: int, amount: float, currency_code: str, session: AsyncSession) -> UserBalance:
    ...
    if amount <= 0:
        raise HTTPException(status_code=400, detail="Kwota wpłaty musi być dodatnia")
...
async def withdraw_funds(user_id: int, amount: float, currency_code: str, session: AsyncSession) -> UserBalance:
    ...
    if not user:
        raise HTTPException(status_code=404, detail="Nie znaleziono użytkownika")
    if amount <= 0:
        raise HTTPException(status_code=400, detail="Kwota wypłaty musi być dodatnia")
    balance_response = await get_user_balance(user_id, session)
    balance = next((b for b in balance_response if b.currency_code == currency_code), None)
    if not balance:
        raise HTTPException(status_code=404, detail=f"Nie znaleziono salda dla waluty {currency_code}")
