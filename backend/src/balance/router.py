...
async def deposit_funds_by_user(amount: float, currency_code: str = "PLN", current_user: User = Depends(get_current_user), session: AsyncSession = Depends(pg_db.get_session)) -> UserBalance:
    ...
    if not currency:
        raise HTTPException(status_code=404, detail=f"Nie znaleziono waluty {currency_code}")
...
@balance_router.get("/balance", response_model=List[UserBalance])
async def get_balance(current_user: User = Depends(get_current_user), session: AsyncSession = Depends(pg_db.get_session)) -> List[UserBalance]:
    """Endpoint do pobierania salda użytkownika."""
    balance = await get_user_balance(current_user.id, session)
    return balance